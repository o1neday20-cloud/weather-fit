  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentOutfitStr) {
      ...
      currentOutfit.forEach((item: any) => {
        if (item.id) outfitIds.push(item.id);
        if (item.style && !outfitStyles.includes(item.style)) outfitStyles.push(item.style);
      });
    }

    // 날씨 정보 가져오기 (weatherApi에서 저장한 마지막 날씨)
    const lastWeather = JSON.parse(localStorage.getItem('lastWeather') || '{}');

    // rating → DB ENUM 값으로 변환
    const feedbackMap: Record<string, '너무추움' | '춥다' | '적당' | '덥다' | '너무더움'> = {
      cold:    '춥다',
      perfect: '적당',
      hot:     '덥다',
    };
