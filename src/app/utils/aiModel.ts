...
export function recommendOutfit(
  ...
  seed?: number  // 새로고침마다 다른 조합을 위한 시드
): ClothingItem[] {
  const recommendation: ClothingItem[] = [];

  let requiredWarmth: number;
  if (feelTemp < 5) requiredWarmth = 14;
  else if (feelTemp < 10) requiredWarmth = 12;
  else if (feelTemp < 15) requiredWarmth = 9;
  else if (feelTemp < 20) requiredWarmth = 6;
  else if (feelTemp < 25) requiredWarmth = 4;
  else requiredWarmth = 2;

  // 옷장 아이템 전체 사용 (localStorage에 저장된 건 모두 내 옷)
  const ownedItems = wardrobe.length > 0 ? wardrobe : [];
  ...
  // 시드 기반 셔플 — 새로고침마다 다른 순서
  const shuffleWithSeed = (arr: ClothingItem[]) => {
  ...
  const sortByStylePreference = (items: ClothingItem[]) => {
    const shuffled = shuffleWithSeed(items);
